    ...
    for (uint i = 0; i < Log_step; i++) {
        ...
        if (p->mark) {
            ...
            printf("[%3d.] '%c' %04x %02x (%04x %02x)\n",
                j, p->kind, p->abus, p->dbus, p->want_abus, p->want_dbus);
            j++;
        }
    }
}
...
#define M(X) { ok = X; if (!ok) goto ERROR; }

void SpoonNMI() {
    bool ok = true;
    ASSERT_NMI(); Mark("Assert Halt");
    ASSERT_HALT();
    for (uint i = 0; i < 20; i++) M(AnyStep())
    ASSERT_NMI(); Mark("Assert NMI");
    for (uint i = 0; i < 2; i++) M(AnyStep())
    RELEASE_NMI(); Mark("Release NMI");
    RELEASE_HALT(); Mark("Release Halt");
    for (uint i = 0; i < 29; i++) M(AnyStep())

    Mark("did NMI plus more");

    ASSERT_HALT(); Mark("AssertHalt, 6");
    for (uint i = 0; i < 6; i++) M(AnyStep())

    RELEASE_HALT(); Mark("ReleaseHalt, 2");
    for (uint i = 0; i < 2; i++) M(AnyStep())
    ...
    ASSERT_HALT(); Mark("Assert Halt & RUNOUT 20");
    for (uint i = 0; i < 20; i++) M(AnyStep())
    SAY('$');
    PrintLog();
    while (1) { sleep_us(1); }
